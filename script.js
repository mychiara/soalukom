...
let previousViewForQuiz = null, previousViewForResult = null;
...
// [... Salin semua DOM Selector yang sudah ada dari script.js sebelumnya ...]
...
// ... (Salin semua fungsi helper otentikasi: generateClientSessionId, clearLocalSessionData, showLoginForm, proceedToApp, heartbeat, startHeartbeat, resetAppStateForLogout)
// Fungsi-fungsi ini sekarang bisa berada di sini, di script.js
...
function showLoginForm(message = "") {
    ...
    licenseMsgElement.style.color = message.toLowerCase().includes("gagal") ? "#FF0000" : "#87CEFA";
...
function proceedToApp(userName, licenseKey, clientSessionId) {
    welcomeUserMessageEl.textContent = `SELAMAT DATANG, ${userName.toUpperCase()}!`;
...
async function heartbeat(licenseKey, clientSessionId) {
    ...
    try {
        ...
        if (docSnap.exists() && docSnap.data().active === true && docSnap.data().activeSessionId === clientSessionId) {
            ...
            showLoginForm("Sesi Anda telah diakhiri.");
        }
...
function resetAppStateForLogout() {
    userName = ""; questions = []; allCategoryQuestions = []; questionStates = []; quizHistory = [];
    currentQuestionIndex = 0; score = 0; isTryOut = false; isReviewMode = false; currentQuizMode = null;
    stopOverallTimer();
    Object.keys(localStorage).forEach(key => { if (key.startsWith('quizProgress_')) localStorage.removeItem(key); });
    const allAppViews = [mainMenuViewContainer, latihanSoalSubmenuContainer, tryoutSubmenuContainer, kategoriLatihanContainer, tryoutSeriusContainer, historyContainer, quizLayoutContainer, resultContainer];
    allAppViews.forEach(view => { if (view) view.classList.add('hidden'); });
}
...
function attachAuthEventListeners() {
    submitLicenseButton.addEventListener("click", async () => {
        // ... (Logika submit lisensi yang ada, tidak perlu diubah)
        ...
        try {
            ...
            if (docSnap.exists() && docSnap.data().active === true) {
                ...
                if (canTakeLicense) {
                    ...
                    licenseMsgElement.textContent = "Lisensi ini sedang aktif digunakan.";
                    licenseMsgElement.style.color = "#FF0000";
                }
            } else {
                licenseMsgElement.textContent = "Lisensi tidak valid atau tidak aktif.";
                licenseMsgElement.style.color = "#FF0000";
            }
        } catch (error) {
    ...
    resetSessionButton.addEventListener("click", async () => {
        // ... (Logika reset sesi yang ada, tidak perlu diubah)
        const licenseKeyToReset = licenseInput.value.trim();
        if (!licenseKeyToReset) {
            resetSessionMsgElement.textContent = "Masukkan kode lisensi yang ingin direset.";
        ...
        try {
            ...
            resetSessionMsgElement.textContent = "Reset berhasil. Silakan coba masuk lagi.";
            resetSessionMsgElement.style.color = "#90EE90";
        } catch (error) {
            resetSessionMsgElement.textContent = "Gagal mereset. Lisensi tidak ditemukan.";
...
function getLocalStorageKey(type) {
    ...
    if (type === 'progress') return `quizProgress_${sessionUserName}_${currentQuizMode}_${currentQuizTitle}`;
    return null;
}
...
async function saveHistory() {
    // 1. Dapatkan data riwayat terbaru dari state global
    if (quizHistory.length === 0) return;
    const latestResult = quizHistory[quizHistory.length - 1];

    // 2. Simpan ke localStorage (seperti sebelumnya)
    try {
    ...
    // 3. Kirim ke Firestore (FITUR BARU)
    const licenseKey = sessionStorage.getItem('licenseKey');
    if (!db || !licenseKey) {
        console.warn("Firestore db atau licenseKey tidak tersedia. Riwayat tidak dikirim ke server.");
        return;
    }
    ...
    try {
        const historyDataForFirestore = {
            ...
            date: serverTimestamp() // Gunakan timestamp server
        };
        ...
        console.error("Gagal mengirim riwayat ke Firestore:", error);
    }
}

// ... (Salin semua fungsi lainnya: renderHistory, saveProgress, loadProgress, clearProgress)
// [.. Kode lainnya dari script.js Anda sebelumnya, TIDAK PERLU DIUBAH ..]
function renderHistory() {
    ...
    quizHistory.slice().reverse().forEach(item => {
        const date = new Date(item.date).toLocaleString('id-ID', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' });
...
function showView(viewToShow) { const allViews = [mainMenuViewContainer, latihanSoalSubmenuContainer, tryoutSubmenuContainer, kategoriLatihanContainer, tryoutSeriusContainer, historyContainer, quizLayoutContainer, resultContainer]; allViews.forEach(view => { if (view) view.classList.toggle('hidden', view !== viewToShow); }); }
...
function startGame(sourceViewElement) { previousViewForQuiz = sourceViewElement; currentQuestionIndex = 0; score = 0; selectedOptionIndex = -1; answerSubmitted = false; isReviewMode = false; userName = sessionStorage.getItem('userName') || "Pengguna"; userNameDisplaySidebar.innerText = userName; if (!questions || questions.length === 0) { alert("Tidak ada soal, kuis tidak dapat dimulai."); showView(sourceViewElement); return; } questionStates = questions.map(() => ({ answered: false, doubtful: false, selectedAnswer: -1 })); if (isTryOut) { timerDisplaySidebar.classList.remove('hidden'); doubtfulButton.classList.remove('hidden'); startOverallTimer(); } else { timerDisplaySidebar.classList.add('hidden'); doubtfulButton.classList.add('hidden'); stopOverallTimer(); } showView(quizLayoutContainer); renderQuestionNavigation(); showQuestion(questions[currentQuestionIndex]); }
function restoreGame(progress, sourceViewElement) { previousViewForQuiz = sourceViewElement; questions = progress.questions; questionStates = progress.questionStates; currentQuestionIndex = progress.currentQuestionIndex; timeLeftOverallSeconds = progress.timeLeftOverallSeconds; currentQuizMode = progress.currentQuizMode; currentQuizTitle = progress.currentQuizTitle; isTryOut = true; isReviewMode = false; score = 0; userName = sessionStorage.getItem('userName') || "Pengguna"; userNameDisplaySidebar.innerText = userName; timerDisplaySidebar.classList.remove('hidden'); doubtfulButton.classList.remove('hidden'); startOverallTimer(); showView(quizLayoutContainer); renderQuestionNavigation(); showQuestion(questions[currentQuestionIndex]); }
...
function showResult() {
    stopOverallTimer();
    completedQuizData = { questions: [...questions], questionStates: [...questionStates], quizName: currentQuizTitle };
    let finalScore = 0, incorrectCount = 0;
    ...
    questions.forEach((q, index) => {
        ...
        if (currentQuizMode === 'tryout-gabungan-semua') { const categoryKey = q.category || 'lainnya'; if (!categoryBreakdown[categoryKey]) categoryBreakdown[categoryKey] = { correct: 0, total: 0 }; categoryBreakdown[categoryKey].total++; if (isCorrect) categoryBreakdown[categoryKey].correct++; }
    });
    if (!isTryOut) finalScore = score;
    if(currentQuizMode !== 'latihan-salah'){
        loadHistory();
        quizHistory.push({ id: Date.now(), quizName: currentQuizTitle, score: finalScore, totalQuestions: questions.length, date: new Date().toISOString() });
        saveHistory(); // INI FUNGSI YANG DIPERBARUI
    }
    resultTitleElement.textContent = `Hasil ${currentQuizTitle}`;
    let message = `<span class="user-name-result">Selamat ${userName}!</span><br>Skor Anda: <span class="score-value">${finalScore}</span> benar dari ${questions.length} soal.<br>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // [.. Salin semua event listener DOMContentLoaded dari script.js sebelumnya ..]
    ...
    if (mainMenuHistoryButton) { mainMenuHistoryButton.addEventListener('click', () => { renderHistory(); showView(historyContainer); }); }
